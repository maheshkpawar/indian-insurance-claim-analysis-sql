...
SELECT
    p.policy_type,
    ROUND(AVG(julianday(c.settlement_date) - julianday(c.intimation_date)), 1) AS avg_days_to_settle,
    MIN(julianday(c.settlement_date) - julianday(c.intimation_date)) AS fastest_settlement_days,
    MAX(julianday(c.settlement_date) - julianday(c.intimation_date)) AS slowest_settlement_days
FROM claims c
JOIN policies p ON p.policy_id = c.policy_id
...
SELECT
    ...
    ROUND(AVG(julianday(settlement_date) - julianday(intimation_date)), 1) AS avg_tat_days
FROM claims
...
SELECT
    ...
    p.policy_type,
    p.end_date,
    CAST(julianday(p.end_date) - julianday('2025-07-02') AS INTEGER) AS days_to_expiry
FROM policies p
JOIN customers cu ON cu.customer_id = p.customer_id
WHERE p.status = 'Active'
  AND julianday(p.end_date) - julianday('2025-07-02') BETWEEN 0 AND 60
...
SELECT
    CASE
        WHEN (2025 - CAST(strftime('%Y', cu.dob) AS INTEGER)) < 30 THEN '18-29'
        WHEN (2025 - CAST(strftime('%Y', cu.dob) AS INTEGER)) < 45 THEN '30-44'
        WHEN (2025 - CAST(strftime('%Y', cu.dob) AS INTEGER)) < 60 THEN '45-59'
        ELSE '60+'
    END AS age_band,
    COUNT(c.claim_id) AS num_claims,
    ROUND(AVG(c.claim_amount), 0) AS avg_claim_amount
